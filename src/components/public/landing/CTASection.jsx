...
export default function CTASection() {
  ...
  const whatsappUrl = `https://wa.me/966552799610?text=${encodeURIComponent(whatsappMessage)}`

  return (
    <section className="gradient-hero relative overflow-hidden py-20 md:py-28">
      {/* Background orb */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute top-1/2 start-1/2 h-[500px] w-[500px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-white/10 blur-[120px]" />
      </div>
      ...
      <div className="container relative mx-auto px-4 text-center">
        <ScrollReveal>
          ...
          <p className="mx-auto mt-5 max-w-lg text-lg text-white/60">{t(c.sub)}</p>
          ...
          <p className="mt-6 text-sm text-white/40">{t(c.note)}</p>
        </ScrollReveal>
      </div>
    </section>
  )
