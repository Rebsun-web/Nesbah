...
export default function BankOfferDetails({ offer, onClose, onEdit }) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                        ...
                        <span className="ml-2 text-gray-900">{offer.application_trade_name || 'N/A'}</span>
                    </div>
                    <div>
                        <span className="font-medium text-gray-700">CR Number:</span>
                        <span className="ml-2 text-gray-900">{offer.application_cr_number || 'N/A'}</span>
                    </div>
                    <div>
                        <span className="font-medium text-gray-700">City:</span>
                        <span className="ml-2 text-gray-900">{offer.application_city || 'N/A'}</span>
                    </div>
                    <div>
                        <span className="font-medium text-gray-700">CR Capital:</span>
                        <span className="ml-2 text-gray-900">
                            {offer.application_cr_capital ? formatCurrency(offer.application_cr_capital) : 'N/A'}
                        </span>
                    </div>
                    <div>
                        <span className="font-medium text-gray-700">Preferred Period:</span>
                        <span className="ml-2 text-gray-900">
                            {offer.application_preferred_period ? `${offer.application_preferred_period} months` : 'N/A'}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
                <h4 className="text-lg font-medium text-gray-900 mb-3">Relationship Manager</h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div>
                        <span className="font-medium text-gray-700">Name:</span>
                        <span className="ml-2 text-gray-900">{offer.relationship_manager_name || 'N/A'}</span>
                    </div>
                    <div>
                        <span className="font-medium text-gray-700">Phone:</span>
                        <span className="ml-2 text-gray-900">{offer.relationship_manager_phone || 'N/A'}</span>
                    </div>
                    <div>
                        <span className="font-medium text-gray-700">Email:</span>
                        <span className="ml-2 text-gray-900">{offer.relationship_manager_email || 'N/A'}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <span className="ml-2 text-gray-900">{formatDate(offer.submitted_at)}</span>
                    </div>
                    <div>
                        <span className="font-medium text-gray-700">Expires:</span>
                        <span className="ml-2 text-gray-900">{formatDate(offer.expires_at)}</span>
                    </div>
                    {offer.accepted_at && (
                        <div>
                            <span className="font-medium text-gray-700">Accepted:</span>
                            <span className="ml-2 text-gray-900">{formatDate(offer.accepted_at)}</span>
                        </div>
                    )}
                    {offer.created_at && (
                        <div>
                            <span className="font-medium text-gray-700">Created:</span>
                            <span className="ml-2 text-gray-900">{formatDate(offer.created_at)}</span>
                        </div>
                    )}
                    {offer.updated_at && (
                        <div>
                            <span className="font-medium text-gray-700">Last Updated:</span>
                            <span className="ml-2 text-gray-900">{formatDate(offer.updated_at)}</span>
                        </div>
                    )}
                </div>
            </div>
