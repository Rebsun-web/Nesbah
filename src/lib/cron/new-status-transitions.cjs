...
const { AnalyticsService } = require('./analytics/analytics-service.cjs');
...
class NewStatusTransitionMonitor {
    ...
    async transitionApplication(applicationId, fromStatus, toStatus, reason, client = null) {
        ...
        try {
            ...
            // Clean up analytics tracking data when auction ends
            if (toStatus === 'approved_leads' || toStatus === 'ignored') {
                try {
                    await AnalyticsService.cleanupAuctionData(applicationId);
                    console.log(`🧹 Cleaned up analytics data for application ${applicationId}`);
                } catch (error) {
                    console.error(`Failed to cleanup analytics data for application ${applicationId}:`, error);
                }
            }
...
module.exports = statusTransitionMonitor;
