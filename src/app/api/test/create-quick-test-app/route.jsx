...
export async function POST(req) {
    try {
        console.log('🧪 Creating new test application with 5-minute expiry...');
        
        const client = await pool.connectWithRetry(2, 1000, 'create-quick-test-app');
        ...
        try {
            ...
            const testTradeName = `Quick Test Business ${new Date().toLocaleTimeString()}`;
            ...
            // 3. Create test POS application with 5-minute expiration
            const submitted_at = new Date();
            // Add 5 minutes to current time for auction end
            const auction_end_time = new Date(submitted_at.getTime() + 5 * 60 * 1000);
            ...
            const posAppResult = await client.query(
                ...
                [
                    user_id, 
                    submitted_at, 
                    'Quick test application for background task testing (5 min expiry)',
            ...
            const timeUntilExpiry = Math.round((auction_end_time - new Date()) / 1000 / 60);
            
            console.log('✅ Quick test application created successfully!');
            ...
            return NextResponse.json({
                success: true,
                message: 'Quick test application created successfully',
                data: {
                    application_id,
                    user_id,
                    testEmail,
                    ...
                    submitted_at_formatted: submitted_at.toLocaleString(),
        ...
        console.error('❌ Error creating quick test application:', error);
        return NextResponse.json(
            { success: false, error: 'Failed to create quick test application' },
            { status: 500 }
        );
    }
}
