...
export async function POST(req) {
    try {
        ...
        const client = await pool.connectWithRetry(2, 1000, 'app_api_admin_users_reset-bank-password_route.jsx_route');
        try {
            // Verify the bank user exists
            const userCheck = await client.query(
                `SELECT u.user_id, u.email, u.user_type, u.entity_name FROM users u WHERE u.user_id = $1 AND u.user_type = 'bank_user'`,
                [user_id]
            );
            ...
            return NextResponse.json({
                success: true,
                message: custom_password ? 'Bank user password set successfully' : 'Bank user password reset successfully',
            ...
            return NextResponse.json(
                { success: false, error: 'Failed to reset bank user password' },
                { status: 500 }
            );
        } finally {
            client.release();
