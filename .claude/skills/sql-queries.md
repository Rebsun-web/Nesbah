...
## Nesbah context — read first

**Primary DB:** PostgreSQL 16. All application queries use raw SQL via `src/lib/db.js`.

**In application code — always parameterised:**
```javascript
// ALWAYS — $1, $2 placeholders via pg driver
await query('SELECT * FROM applications WHERE bank_id = $1 AND status = $2', [bankId, status])

// NEVER — string interpolation
await query(`SELECT * FROM applications WHERE bank_id = ${bankId}`)
```

**Key tables (inferred from codebase context):**
- `applications` — POS financing applications from businesses
- `businesses` — business_user accounts
- `banks` — bank institutions
- `users` / `bank_users` / `business_users` — role-specific user tables
- `offers` — bank offers on applications
- `background_tasks` / auction-related tables

**For DA/DS analytical work** (outside the app, e.g. Databricks, BigQuery, Snowflake):
use the dialect-specific sections below. Parameterisation rules don't apply there.
