...
async function createAnalyticsTables() {
    ...
    try {
        console.log('🔧 Creating analytics tables for bank application tracking...\n');
        ...
        // Create bank_offer_submissions table to track when banks submit offers
        ...
        // Create time_metrics table to store calculated metrics for each bank
        ...
        await client.query(`
            CREATE INDEX IF NOT EXISTS idx_time_metrics_bank_id ON time_metrics(bank_user_id);
            CREATE INDEX IF NOT EXISTS idx_time_metrics_calculation_date ON time_metrics(calculation_date);
            CREATE INDEX IF NOT EXISTS idx_time_metrics_metric_type ON time_metrics(metric_type);
        `);
        console.log('✅ Indexes created for time_metrics');

        // Create a function to calculate and store time metrics
        ...
        await client.query(`
            ...
            BEGIN
                ...
                LOOP
                    ...
                    JOIN applications a ON bav.application_id = a.application_id
        ...
        // Create a function to clean up temporary data when auctions end
        ...
        console.log('\n📋 Tables created:');
        console.log('   - bank_application_views (tracks when banks view applications)');
        console.log('   - bank_offer_submissions (tracks when banks submit offers)');
        console.log('   - time_metrics (stores calculated metrics for each bank)');
        console.log('\n🔧 Functions created:');
        console.log('   - calculate_bank_time_metrics() (calculates daily metrics)');
        console.log('   - cleanup_auction_tracking_data() (cleans up old tracking data)');
...
if (require.main === module) {
    createAnalyticsTables()
        ...
        .catch((error) => {
            console.error('💥 Analytics tables creation failed:', error);
            process.exit(1);
        });
}
